...
// Flat degree semitones (b7, b3, b6, b2)
const FLAT_DEGREE_ST = {7:10,3:3,6:8,2:1,5:6};
...
// Normalise "b7" → "♭7" for display in chart cells (not in transposition mode)
function normaliseFlatNotation(cell) {
  // Replace leading lowercase b before a digit with ♭ symbol
  return cell.replace(/^b(\d)/, "♭$1");
}
...
// Enhanced transposeCell — handles b7, b3, b6, b2, b5 flat degree notation
function transposeCell(cell,key) {
  const root=KEY_TO_IDX[key];
  if(root===undefined) return cell;

  // Match flat degree: b7, b3, b6, b2, b5 (optionally followed by slash bass)
  // Also match normal degrees: 1, 6m, 5sus, 1/3 etc.
  return cell.replace(
    /\b(b(\d)|(\d))(m|sus|maj|dim|aug|add\d*)?(\/(\d)(m|sus)?)?/g,
    (match,_full,flatDeg,normalDeg,mod,slash,bd,bm)=>{
      let semis;
      let isFlat=false;
      if(flatDeg!==undefined){
        // Flat degree like b7
        semis=FLAT_DEGREE_ST[+flatDeg];
        isFlat=true;
      } else {
        semis=DEGREE_ST[+normalDeg];
      }
      if(semis===undefined) return match;
      const ch=CHROMATIC[(root+semis)%12]+(mod||"");
      if(slash&&bd){
        const bs=DEGREE_ST[+bd];
...
// Normalise chart display (b7 → ♭7) without transposing
function normaliseChartLine(line) {
  if(!line||!line.includes("|")) return line;
  return line.split("|").map((seg,i,arr)=>
    i===0||i===arr.length-1?seg:" "+normaliseFlatNotation(seg.trim())+" "
  ).join("|");
}
...
async function fetchFromCloud(){
  ...
  return{songs:sd?sd.map(r=>({id:r.id,...r.data})):null,setlist:sl?{ids:sl.song_ids}:null};
}
...
const noSelect={userSelect:"none",WebkitUserSelect:"none",MozUserSelect:"none"};

// ─── Key pickers ──────────────────────────────────────────────────────────────
function buildKeyGrid(value,onChange,compact=false){
  ...
  const p=compact?"7px 2px":"9px 4px",fs=compact?14:16,gap=compact?5:6;
  return(
    <div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap,marginBottom:8}}>
        {NATURAL_KEYS.map(n=><button key={n} onClick={()=>selectNatural(n)} style={{padding:p,borderRadius:8,border:`1px solid ${natural===n?T.accent:T.border}`,background:natural===n?T.accentBg:T.bg,color:natural===n?T.accent:T.text,fontSize:fs,fontWeight:natural===n?700:400,cursor:"pointer",fontFamily:T.mono}}>{n}</button>)}
      </div>
      ...
      <div style={{textAlign:"center",marginTop:compact?8:10,fontSize:12,color:T.textMuted,fontFamily:T.mono}}>Selected: <span style={{color:T.accent,fontWeight:700}}>{value||"—"}</span></div>
    </div>
  );
}
function KeyPicker({value,onChange}){return buildKeyGrid(value,onChange,false);}
function LiveKeyPicker({value,onChange}){
  return(
    <div style={{position:"absolute",top:"calc(100% + 6px)",left:0,zIndex:300,background:T.surface,border:`1px solid ${T.border}`,borderRadius:12,padding:12,minWidth:260,boxShadow:"0 8px 24px rgba(0,0,0,0.5)"}}>
      {buildKeyGrid(value,onChange,true)}
    </div>
  );
}
...
// ─── ChartLine — b7 displayed as ♭7 ──────────────────────────────────────────
function ChartLine({line,chordMode,liveKey}){
  ...
  if(line.type==="bars"){
    const rawCells=line.content.split("|").map(c=>c.trim()).filter(Boolean);
    const cells=rawCells.map(cell=>{
      if(chordMode&&liveKey) return transposeCell(cell,liveKey);
      return normaliseFlatNotation(cell);
    });
    return(
      <div style={{display:"flex",gap:4,marginBottom:4,flexWrap:"wrap"}}>
        {cells.map((cell,i)=>{
          const isEmpty=cell==="—"||cell==="-";
          return<div key={i} style={{background:isEmpty?"transparent":T.accentBg,border:`1px solid ${isEmpty?T.borderLight:T.accentBorder}`,borderRadius:6,padding:"5px 6px",fontFamily:T.mono,fontSize:13,fontWeight:700,color:isEmpty?T.textFaint:T.accent,minWidth:28,textAlign:"center",flex:"1 1 auto",maxWidth:60}}>{cell}</div>;
        })}
      </div>
    );
  }
...
// ─── Drag-to-Reorder — ghost via React portal at body level ──────────────────
const ROW_H=72;
...
function DraggableSetlist({songs,keyOverrides,onKeyChange,onRemove,onReorder,onTapToPlay}){
  ...
  const startDrag=(id,clientX,clientY)=>{
    ...
    activeId.current=id;
    setDraggingId(id);
    // Ghost centered exactly on finger
  ...
  const moveDrag=(clientX,clientY)=>{
    ...
    // Update ghost position — centered on finger
    setGhostPos({x:clientX,y:clientY});
    // Reorder preview
    ...
    // Auto-scroll
    const scrollEl=containerRef.current?.closest("[data-scroll]");
    if(scrollEl){
      const rect=scrollEl.getBoundingClientRect();
      if(clientY>rect.bottom-70)scrollEl.scrollTop+=8;
      else if(clientY<rect.top+70)scrollEl.scrollTop-=8;
    }
  };
  ...
  const onTouchStart=(id,e)=>{
    const t=e.touches[0];
    longPressRef.current=setTimeout(()=>startDrag(id,t.clientX,t.clientY),400);
  };
  ...
  // Mouse
  const onMouseDown=(id,e)=>{longPressRef.current=setTimeout(()=>startDrag(id,e.clientX,e.clientY),400);};
  const onMouseMoveGlobal=useCallback(e=>{if(!isDragging.current)return;moveDrag(e.clientX,e.clientY);},[orderedIds]);
  const onMouseUpGlobal=useCallback(()=>{clearTimeout(longPressRef.current);endDrag();},[orderedIds]);
  ...
  const draggingKeyDisplay=draggingId?(keyOverrides[draggingId]||draggingSong?.key||""):null;

  // Ghost width matches container
  const[containerW,setContainerW]=useState(340);
  const[containerL,setContainerL]=useState(0);
  useEffect(()=>{
    if(!containerRef.current)return;
    const rect=containerRef.current.getBoundingClientRect();
    setContainerW(rect.width);
    setContainerL(rect.left);
  },[draggingId]);

  // Portal target
  const portalEl=typeof document!=="undefined"?document.getElementById("drag-portal")||document.body:null;

  return(
    <div ref={containerRef} style={{...noSelect,position:"relative"}}>
      ...
      {orderedSongs.map((song,visualIdx)=>{
        ...
        return(
          <div key={song.id} style={{opacity:isBeingDragged?0:1,transition:"opacity 0.15s",marginBottom:7}}>
            <SetlistRow
              song={song}
      ...
      {/* Ghost rendered via portal — completely outside any stacking context */}
      {draggingId&&draggingSong&&portalEl&&createPortal(
        <div style={{
          position:"fixed",
          // Center ghost on finger: subtract half row height vertically, half width horizontally
          top:ghostPos.y-ROW_H/2,
          left:containerL,
          width:containerW,
          zIndex:99999,
          pointerEvents:"none",
...
function SongModal({existing,onClose,onSave,onDelete}){
  ...
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.75)",zIndex:200,display:"flex",alignItems:"flex-end",justifyContent:"center",touchAction:"none"}} onTouchMove={e=>{e.preventDefault();e.stopPropagation();}} onClick={onClose}>
      <div onClick={e=>e.stopPropagation()} onTouchMove={e=>e.stopPropagation()} style={{background:T.surface,borderRadius:"16px 16px 0 0",padding:"20px 20px 40px",width:"100%",maxWidth:560,maxHeight:"90vh",overflowY:"auto",WebkitOverflowScrolling:"touch",border:`1px solid ${T.border}`,borderBottom:"none",touchAction:"pan-y"}}>
        ...
        <label style={lbl}>Chord Chart <span style={{color:T.textFaint,fontWeight:400}}>(use b7, b3 etc. for flat degrees)</span></label>
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.85)",zIndex:150,display:"flex",flexDirection:"column",touchAction:"none"}} onClick={onClose} onTouchMove={e=>{e.preventDefault();e.stopPropagation();}}>
      <div onClick={e=>e.stopPropagation()} onTouchMove={e=>e.stopPropagation()} style={{flex:1,display:"flex",flexDirection:"column",background:T.bg,marginTop:"env(safe-area-inset-top,44px)",touchAction:"pan-y"}}>
        ...
        <div style={{flex:1,overflowY:"auto",padding:"14px 18px 40px"}}>
          {formatChart(song.chart).map(l=><ChartLine key={l.id} line={l} chordMode={chordMode} liveKey={liveKey}/>)}
        </div>
      </div>
...
function PerformanceView({songs,currentIndex,onIndexChange,onExit,onUpdateSong,keyOverrides,onKeyOverrideChange}){
  ...
  const handleKeyChange=k=>{
    setLocalKeys(p=>({...p,[song.id]:k}));
    setShowKeyPicker(false);
    onKeyOverrideChange(song.id,k);
  };
  const saveNote=()=>{onUpdateSong(song.id,{note:liveNote});setEditingNote(false);};
...
export default function App(){
  ...
  return(
    <div style={{height:"100vh",background:T.bg,fontFamily:T.font,color:T.text,display:"flex",flexDirection:"column",overflow:"hidden"}}>
      ...
      {/* Swipeable content */}
      <div style={{flex:1,overflow:"hidden",position:"relative"}} onTouchStart={onTabTS} onTouchMove={onTabTM} onTouchEnd={onTabTE}>
        <div style={{display:"flex",width:"200%",height:"100%",transform:`translateX(${tabStripX}px)`,transition:tabSettling?"transform 0.28s cubic-bezier(0.25,1,0.5,1)":"none",willChange:"transform"}}>
          ...
          <div data-scroll="true" style={{width:"50%",height:"100%",overflowY:"auto",WebkitOverflowScrolling:"touch"}}>
            <div style={{maxWidth:560,margin:"0 auto",padding:"0 16px 140px",boxSizing:"border-box"}}>
              {setlistSongs.length===0
                ...
                :<DraggableSetlist songs={setlistSongs} keyOverrides={keyOverrides} onKeyChange={setKeyOverride} onRemove={removeSong} onReorder={reorderSetlist} onTapToPlay={startFromIndex}/>
              }
            </div>
          </div>
